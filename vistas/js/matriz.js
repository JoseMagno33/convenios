...
function init(){
	...
	document.getElementById('tablamatriz').style.display = 'none';
...
function pdf()
{
	...
	//window.open("MPDF/REPORTES/imprimir_ticket_tramite.php?codigo="+5+"", "_blank", 'width=950,height=600'); return false;
...
function asignar_entidad_2(id_pais)
{

	$.ajax({
		type:"POST",
		url:"../ajax/matriz.php?op=encontrarentidad",
		data:{id_pais:id_pais},
		dataType: 'html',
...
function listar_matriz_tabla()
{
	document.getElementById('vista_a').style.display = 'none';
	document.getElementById('tablamatriz').style.display = 'contents';
	...
	$.ajax({
		type:"POST",		
		url:"../ajax/matriz.php?op=crearmatrizporplan",
		data:{id_plan:id_plan,id_pais:id_pais},
		dataType: 'html',
		success:function(data){
	...
	$.post("../ajax/matriz.php?op=mostrar",{id_matriz : id_matriz}, function(data, status)
	{
		data = JSON.parse(data);
		   ...
		   console.log(data);
					...
					$('#entidad_responsable_2').val(data.entidad_responsable);
...
//importante:id_cliente, est se envia por post via ajax
	 ...
	 function eliminar(id_matriz){
		...
		//Enviar datos a reporte

		function reporte_matriz(id_plan,id_pais){
			var direccion = "../vistas/prueba.php?plan="+id_plan+"&pais="+id_pais;

			console.log(id_pais);
			console.log(id_plan);
			window.open(direccion,"popup","width=800,height=800,left=20,top=20"); 
			
			     /*   $.ajax({
						url:"../vistas/prueba.php",
						method:"POST",
						data:{id_pais:id_pais,id_plan:id_plan},
	
						success:function(data)
						{
							//$('#resultadoajax').html(data);
							//alert(data);
							//$("#resultados_ajax").html(data);
							//$("#cliente_data").DataTable().ajax.reload();
							window.open("../vistas/prueba.php?plan='.id_plan.'","popup","width=300,height=300,left=20,top=20"); 
						}
					});*/
