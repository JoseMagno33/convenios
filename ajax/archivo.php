<?php
  ...
  // $imagen = isset($_POST["hidden_nombre_archivo"]);
           ...
           case "guardaryeditar":
	           ...
	           importante: se debe poner el $_POST sino no funciona*/
	          	...
	          	if(empty($_POST["id_archivo"])){
		 				...
		 				$archivo->registrar_archivo($id_pais,$descripcion,$tipov,$nombre_archivo,$id_usuario);
			       	   	$messages[]="Se registró correctamente";

			    }//cierre de empty
	            else {
	            	/*si ya existe entonces editamos matriz*/
						$pais->editar_archivo($id_archivo,$id_pais,$descripcion,$tipov,$nombre_archivo,$id_usuario);
     ...
     case 'mostrar':
			...
			$datos=$archivo->get_archivo_por_id($_POST["id_archivo"]);



    				foreach($datos as $row)
    				{
						//$output["id_archivo"] = $row["id_archivo"];
    					$output["id_pais"] = $row["id_pais"];
						$output["descripcion"] = $row["descripcion"];
						//$output["nombre_archivo"] = $row["nombre_archivo"];
						$output["tipov"] = $row["tipov"];
	...
	break;
     ...
     foreach($datos as $row)
		 ...
		 <a href="upload/'.$row["nombre_archivo"].'" target="_blank">Ver Archivo</a>
				 ...
				 $sub_array[] = '<button type="button" onClick="mostrar('.$row["id_archivo"].');" id="'.$row["id_archivo"].'" class="btn btn-warning btn-md"><i class="glyphicon glyphicon-edit"></i> Editar</button>';
                 $sub_array[] = '<button type="button" onClick="eliminar('.$row["id_archivo"].');" id="'.$row["id_archivo"].'" class="btn btn-danger btn-md"><i class="glyphicon glyphicon-edit"></i> Eliminar</button>';
     ...
     case "listararchivo":
		...
		$datos=$archivo->get_archivo_por_pais_tipov($_POST["id_pais"],$_POST["tipov"]);
		...
		foreach($datos as $row)
			...
			<a href="upload/'.$row["nombre_archivo"].'">Ver Archivo</a>
						   ';
					   }
					   else
					   {
						   $sub_array[] = '<button type="button" id="" class="btn btn-primary btn-md"><i class="fa fa-picture-o" aria-hidden="true"></i> No existe Archivo</button>';
					   }
